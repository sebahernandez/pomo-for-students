...
El sistema SHALL ofrecer tres modos de temporizador — Enfoque (`work`), Descanso Corto (`shortBreak`) y Descanso Largo (`longBreak`) — cada uno con su propia duración configurable. Al seleccionar un modo, el temporizador SHALL cargar la duración de ese modo y quedar en estado inactivo (`idle`).
